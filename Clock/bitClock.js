...
let makeBitTimer = (container,num)=>{
    ...
    if(ran!==0){[...container.getElementsByTagName('div')].forEach(element=>{element.remove()})}
    Array(11).fill(null).forEach(item=>{divs.push(document.createElement('div'))});
    divs.forEach((div,index)=>{
        div.innerText = bitNum[index]===undefined?0:bitNum[index];
